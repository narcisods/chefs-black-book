...
module.exports = {
	getProfile: async (req, res) => {
		...
		const latest = await Recipe.find({ user: req.user.id })
			.sort({ _id: -1 })
			.limit(limitNumber);
		const food = { latest };
		try {
			res.render('profile.ejs', {
				title: `Chef's Black Book`,
				user: req.user,
				food,
				signup: true,
			});
	...
	getCatagories: async (req, res) => {
		try {
			res.render('catagories.ejs', {
				title: `Chef's Black Book - Catagories`,
				user: req.user,
			});
		} catch (err) {
			console.log(err);
	...
	getComponents: async (req, res) => {
		try {
			...
			const component = await Recipe.find({
				category: 'Component',
				user: req.user.id,
			})
				.sort({ name: 1 })
			...
			res.render('component.ejs', {
				title: `Chef's Black Book - Components`,
				user: req.user,
				component,
			});
		} catch (err) {
	...
	getSauces: async (req, res) => {
		try {
			const limitNumber = 25;
			const sauces = await Recipe.find({
				user: req.user.id,
				category: 'Sauce',
				user: req.user.id,
			})
	...
	getFinal: async (req, res) => {
		try {
			const limitNumber = 25;
			const final = await Recipe.find({ user: req.user.id, category: 'Final' })
				.sort({ name: 1 })
				.limit(limitNumber);
