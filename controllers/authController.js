...
exports.postLogin = (req, res, next) => {
	...
	req.body.email = validator.normalizeEmail(req.body.email, {
		//Creates req.body.email and assigns it a normailzedEmail using validator
		gmail_remove_dots: false, //Method that removes dots from email set to false
	});

	passport.authenticate('local', (err, user, info) => {
		//passport method to authenticate user
		if (err) {
			//errors
			return next(err);
		}
...
exports.getSignup = (req, res) => {
	...
	res.render('signup', {
		//if not logged in, renders signup page
...
exports.postSignup = (req, res, next) => {
	...
	if (!validator.isLength(req.body.password, { min: 8 }))
		//Checks password length min of 8
		validationErrors.push({
			msg: 'Password must be at least 8 characters long', //Sends msg to valdationErrors variable
		});
	...
	if (validationErrors.length) {
		//Checks how many errors in valdation errors
	...
	User.findOne(
		//finds one user to check if user already exsist
